...
class FODESUtils:
    ...
    def ensureIndices(self):
        ...
        fodApplicationsExists = False
        fodReleasesExists = False
        fodCountsExists = False
        fodScansExists = False
        fodScanSummaryExists = False
        fodRelIssuesExists = False
        fodAllExists = True

        for _idx in indices:

            if _idx['index'] == 'fodapplications':
                fodApplicationsExists = True
            elif _idx['index'] == 'fodreleases':
                fodReleasesExists = True
            elif _idx['index'] == 'fodcounts':
                fodCountsExists = True
            elif _idx['index'] == 'fodscans':
                fodScansExists = True
            elif _idx['index'] == 'fodscansummary':
                fodScanSummaryExists = True
            elif _idx['index'] == 'fodrelissues':
                fodRelIssuesExists = True
        ...
        if not fodCountsExists:
            logging.info('fodcounts does not exists')
            fodAllExists = False

        if not fodScansExists:
            logging.info('fodscans does not exists')
            fodAllExists = False

        if not fodScanSummaryExists:
            logging.info('fodscansummary does not exists')
            fodAllExists = False

        if not fodRelIssuesExists:
            logging.info('fodrelissues does not exists')
            fodAllExists = False
