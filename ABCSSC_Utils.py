...
class sscUtils:
    ...
    def getProjectVersionByProjectVersionId(self, id):

        _url = 'https://fortify.1dc.com/ssc/api/v1/projectVersions/{}?fields=project,id,issueTemplateId,currentState,name'.format(id)

        response = requests.get(_url, auth=HTTPBasicAuth(self.sscUser, self.sscPassword), headers=self.headers)
        ProjVersion = json.loads(response.text)
